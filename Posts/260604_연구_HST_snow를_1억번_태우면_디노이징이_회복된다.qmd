...
같은 $t^*$ 의 snow 연산자 $B_{t^*}=-\tfrac12 J C_{t^*} J$ 를 고유분해해서, 큰 고유값 쪽 고유벡터 두 개로 노드를 찍어봄. 이게 snow embedding 임. 두 가지를 나란히 봄 — 왼쪽은 snow 거리(그래프)만, 가운데는 snow 거리에 신호 거리 $(s_i-s_j)^2$ 를 섞은 **joint** 거리임.

![](attachments/260604_a7f3c1_03.png){fig-align="center"}

왼쪽(그래프만)은 **완벽한 원**임. 노드 번호를 안 줬는데도 snow 거리가 링의 위상을 다 담고 있음 — 거리만으로 그래프가 복원됨. 그런데 이건 신호를 안 본 graph-domain 결과라, 신호가 뭐든 항상 같은 원이 나옴.

가운데(joint)가 진짜 보고 싶었던 거임. 신호의 점프를 거리에 넣으면 원이 **점프 자리에서 두 호로 끊어짐** — 신호가 같은 평탄 구간끼리는 가깝고, 점프를 사이에 둔 노드끼리는 멀어지기 때문임. 위상(링)과 데이터(점프)가 한 그림에 같이 드러나는 게 joint-domain 의 핵심임. graph 만 보면 끊김이 안 보이고, 신호만 보면 위상이 안 보이는데, snow 거리에 둘을 섞으니 둘 다 보임.
...
- 임베딩에서 graph 만 보면 깨끗한 원, 신호를 섞으면 점프 자리에서 끊어진 두 호 — 위상과 데이터가 한 그림에 같이 사는 게 joint-domain 의 증거임.
